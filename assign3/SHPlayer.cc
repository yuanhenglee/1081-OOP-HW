...
void
SHPlayer::showCards(char const *sideStr) const
{
  char str[kCardWidth+1];
  for( int i = 0; i < kCardHeight ; ++i ){
    for( int CardCount = 0; CardCount < numOfCard ; ++CardCount ){
      if( CardCount == 0 )
        cout<<sideStr[i]<<" ";
      for( int j = 0; j < kCardWidth ; ++j ){
        if( CardCount == 0 && !showFirst )
          ...
          str[j] = cardAbbrev[ cards[CardCount].getSuit() ];
        else
      ...
      if( CardCount == numOfCard-1 )//last card in a row
        cout<<endl;
      else
        cout<<" ";
    }
